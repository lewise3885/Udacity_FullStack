...
#	urllib: used to strap or grab the needed urls to be placed into a variable for later use
#	os: more of a just incase lib to connect to Operating System's command line <= but was never used in this build
#	webbrowser: only purpose in this build was to open a browser to my compiled webpage
#	re: used to search and isolate current portions of the straped HTML via urllib main to acquire needed values
import urllib, os, webbrowser, re
...
def gms(movies):
	...
	m = int(0)
	for movie in movies:
		movielink = urllib.urlopen('http://www.imdb.com/title/'+movie['imdbId']+'/?ref_=tt_rec_tti')
		...
		for description in re.findall('<p itemprop="description">([^<]+?)</p>', movielink):
			ma[m]['description']=description
		for rating in re.findall('<div class="titlePageSprite star-box-giga-star">([^<]+?)</div>', movielink):
			ma[m]['rating']=rating_stars(rating)
		imgs = re.compile('"img_primary">(.*?)</div>',re.DOTALL).findall(movielink)
		imgs = re.findall('src="(.*?)"',imgs[0])
		ma[m]['image'] = '<img src="'+str(imgs[0])+'" />'
		ma[m]['mlink'] = 'http://www.imdb.com/title/'+movie['imdbId']+'/?ref_=tt_rec_tti'
		mhtml += movie_content.format(movie_image=ma[m]['image'] , movie_title=ma[m]['title'] , movie_description=ma[m]['description'] , movie_rating=ma[m]['rating'] , videoID=movie['youtubeId'] , hyperlink=ma[m]['mlink'])
		m = m+1
	return mhtml 
...
def addext(filetype,files):
	...
	if str(type(files)) == "<type 'str'>":
		if str(filetype) == 'css':
			result = result + '<link rel="stylesheet" type="text/css" href="' + files + '" />'+'\n'
		elif str(filetype) == 'js':
			result = result + '<script type="text/javascript" src="' + files + '" ></script>'+'\n'
	elif str(type(files)) == "<type 'list'>":
		listCount = int(0)
		while listCount < len(files):
			if str(filetype) == 'css':
				result = result + '<link rel="stylesheet" type="text/css" href="' + files[int(listCount)] + '" />'+'\n'
			elif str(filetype) == 'js':
				result = result + '<script type="text/javascript" src="' + files[int(listCount)] + '" ></script>'+'\n'
			listCount = listCount + 1
	else:
		result = 'error'
	result = result + '<!-- E: Imported External ' + str(filetype).upper() + ' -->'+'\n'
	return str(result)
...
movie_content = '''<div class="movie_item" data-video="{videoID}" >
	...
	<div class="movie_itemTitle" data-movieTitle="{movie_title}" data-movieDescription="{movie_description}">{movie_title}
...
headerJsExt = addext('js',['https://code.jquery.com/jquery-2.1.4.min.js','https://code.jquery.com/ui/1.11.4/jquery-ui.min.js','https://www.youtube.com/iframe_api'])
...
/* 
	call_youtube_video function works hand and hand with the external imported library
	youtube iframe api {YT.Player} to produce html5 playback to I can isolate when the 
	video stop and to close the trailer window 
*/
function call_youtube_video(elem) {
	...
	if(elem.attr('data-video').length == 0){alert("Sorry this Movie does not have a trailer. But you can view it's description and info at imdb's link below")}
  player = new YT.Player($(".vid_trailer_video")[0], {
    ...
    events: {
		'onReady': function(e){ 
			...
			$('.vid_trailer').find('#movieTitle').text(m_data.attr('data-movieTitle'))
			$('.vid_trailer').find('#movieDescription').text(m_data.attr('data-movieDescription'))
			$('.vid_trailer').find('#movieRating').text(m_data.find('.movie_itemRating').text())
		},
...
$(function(){
	/* Acknowledge when the user is over the trailer popup versus the body {grayed area} */
	$('.vid_trailer').mouseenter(function(){
		if($('body').hasClass('mouseintrailer')){}else{$('body').addClass('mouseintrailer')}
	}).mouseleave(function(){
		if($('body').hasClass('mouseintrailer')){$('body').removeClass('mouseintrailer')}
	});
	/* on click of the body; if a movie trailer is active execute the close trailer function */
	$('body').click(function(){
		if($(this).hasClass('mouseintrailer')){ }else{ if($(this).hasClass('playingvid')){$('.close_vid').click();} }
	});
	/* Once a movie has been selected run the trailer function : call_youtube_video */
	...
	$('.close_vid').click(function(){
		...
		$(this).parent().find('.vid_wrapper').html('<div class=vid_trailer_video ></div>')
	});
});	
</script>
...
<html>
	...
	<body>
		...
		<div class="vid_trailer">
			<!-- Wrapper being dynamically rebuilt via the clients selection -->
...
web = web.format(headerCss_ext=headerCssExt , headerJs_ext=headerJsExt , headerCss=headerCss , headerJs=headerJs , movies=myMovies)

#create and or open the file needed to input all compiled string data from the web variable

webpage = open('movies.html','w')

#if the current file opened if empty or not empty; truncate from my understanding removes all data in file

webpage.truncate()

#write newly compiled data from the variable web to the currently opened, completely empty file AKA: movies.html

webpage.write(web)

#the close command will save and close all modications if any was committed and no modications it was just simpley close the file

webpage.close()

#this will try to open a new browser tab to your default and or specified browser aswell as to open the currently close file : movies.html
